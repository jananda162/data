    ...
    if (empty($user_id) || empty($department_id) || empty($category_id) || empty($issue_description)) {
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                // Clear form fields after successful submission by redirecting or clearing POST.
                // For now, just a message. A redirect would be cleaner.
                $_POST = array(); // Clear POST data to prevent re-submission issues on refresh
                                 // and to clear form fields (if not pre-filling from GET)
                // Reset selected device info if it was from POST
                $selected_device_id = '';
...
// --- Pre-fill device information if coming from search_devices.php ---
$selected_device_id = '';
$selected_device_info = 'None selected. Use search.';
...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['device_id'])) {
    ...
    if (isset($_GET['device_info'])) {
        ...
        // Fetch device info if only ID is provided (less ideal but a fallback)
        ...
        if ($dev_stmt) {
            ...
            if ($dev_row = $dev_result->fetch_assoc()) {
                ...
                $selected_device_id = ''; // ID not found
...
<div class="card">
    ...
    <div class="card-body">
        ...
        <?php endif; ?>
        <?php if (!empty($message) && $_SERVER["REQUEST_METHOD"] == "POST"): // Show submission message only after POST ?>
            <div class="alert <?php echo strpos($message, 'Error:') === 0 ? 'alert-danger' : 'alert-success'; ?>">
                <?php echo htmlspecialchars($message); ?>
            </div>
        <?php endif; ?>

        <form action="log_issue.php" method="post" id="logIssueForm">
...
<script>
    function searchDeviceSimple() { // This function is no longer used by default, but kept for reference or fallback
        const serialNumber = document.getElementById('device_serial_search').value;
        if (serialNumber.trim() === "") {
            alert("Please enter a serial number to search.");
            return;
        }
        const searchUrl = `search_devices.php?search_term=${encodeURIComponent(serialNumber)}`;
...
$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('#modal_department_name').val(''); // Clear input
                    $('#modalDepartmentMessage').html(''); // Clear message
                    showPageMessage('Department "' + response.name + '" added successfully!', true);
                } else {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('#modal_category_name').val(''); // Clear input
                    $('#modalCategoryMessage').html(''); // Clear message
                    showPageMessage('Category "' + response.name + '" added successfully!', true);
                } else {
    ...
    $('#ajaxSearchDeviceBtn').on('click', function() {
        ...
        if (serialNumber === "") {
            alert("Please enter a serial number to search.");
            $('#device_search_results_ajax').html(''); // Clear previous results
            return;
        }

        $.ajax({
            url: 'search_devices.php',
            type: 'GET',
    ...
    $(document).on('click', '.select-device-btn', function() {
        ...
        $('#device_search_results_ajax').html(''); // Clear search results
        $('#device_serial_search').val(deviceInfo.split(' - ')[1]); // Optionally prefill search with selected serial
    });
    ...
    $('#addDepartmentModal, #addCategoryModal').on('hidden.bs.modal', function () {
        $(this).find('.alert').remove(); // More robust than .html('')
